...
class Sigmoid(ScalarFunction):
    ...
    @staticmethod
    def forward(ctx: Context, a: float) -> float:
        sigm_a_value = operators.sigmoid(a)
        ctx.save_for_backward(sigm_a_value)
        return float(sigm_a_value)

    @staticmethod
    def backward(ctx: Context, d_output: float) -> float:
        (sigm_a_value, ) = ctx.saved_values
        return float(d_output * sigm_a_value * (1 - sigm_a_value))
